...
df['EMA'] = ta.ema(df['Adj Close'], length=150)
df.tail()
...
for row in range(backcandles, len(df)):
    ...
    for i in range(row-backcandles, row+1):
        if max(df['Open'].iloc[i], df['Adj Close'].iloc[i])>=df.EMA[i]:
            dnt=0
        if min(df['Open'].iloc[i], df['Open'].iloc[i])<=df.EMA[i]:
            upt=0
    if upt==1 and dnt==1:
        EMAsignal[row]=3
    elif upt==1:
...
def detect_structure(candle, backcandles, window):
    ...
    highs = localdf['High'][localdf['isPivot'] == 1].tail(3).values
    idxhighs = localdf['High'][localdf['isPivot'] == 1].tail(3).index
    lows = localdf['Low'][localdf['isPivot'] == 2].tail(3).values
    idxlows = localdf['Low'][localdf['isPivot'] == 2].tail(3).index
